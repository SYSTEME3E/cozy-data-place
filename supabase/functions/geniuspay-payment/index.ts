...
serve(async (req: Request) => {
  ...
  try {
    ...
    // GeniusPay redirigera l'utilisateur vers ces URLs
    const success_url = `${APP_URL}/payment/callback?status=success&type=${type}&user_id=${user_id}`;
    const error_url   = `${APP_URL}/payment/callback?status=failed&type=${type}&user_id=${user_id}`;
