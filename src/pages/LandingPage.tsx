...
import {
  ...
  Network, GraduationCap, QrCode, Home,
} from "lucide-react";
...
const FEATURES = [
  ...
  { icon: Network, color: "#8b5cf6", bg: "#f5f3ff", title: "Réseau MLM & Parrainage", tag: "Disponible", desc: "Construisez votre réseau de filleuls et gagnez des commissions automatiques sur 3 niveaux. Plus votre réseau grandit, plus vos revenus passifs augmentent chaque semaine.", points: ["Niveau 1 : 30% de commission", "Niveau 2 : 10% de commission", "Niveau 3 : 5% de commission", "Retrait Mobile Money"] },
  { icon: GraduationCap, color: "#f59e0b", bg: "#fffbeb", title: "Formations & Affiliation", tag: "Disponible", desc: "Accédez à des formations exclusives et gagnez 30% de commission sur chaque vente générée via votre lien affilié. Achetez, apprenez, revendez et gagnez.", points: ["Formations vidéo & PDF", "30% de commission affilié", "Lien affilié unique", "Stats de vente en temps réel"] },
  { icon: Sparkles, color: "#ec4899", bg: "#fdf2f8", title: "Nexora Academy", tag: "Disponible", desc: "Contenu exclusif pour les membres NEXORA. Tutoriels, stratégies business et formations pratiques pour entrepreneurs africains. Accès illimité inclus.", points: ["Contenu exclusif membres", "Mises à jour régulières", "Accès illimité", "Stratégies business Africa"] },
...
export default function LandingPage() {
  ...
  const [reviews, setReviews] = useState<{ name: string; country: string; text: string; stars: number }[]>([
    ...
    { name: "Eric Mensah", country: "🇨🇮 Côte d'Ivoire", text: "Le système MLM est génial. J'ai déjà 18 filleuls et je touche des commissions automatiquement. PayLink m'a aussi changé la vie pour encaisser mes clients.", stars: 5 },
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#0a0f1e] text-gray-900 dark:text-gray-100 overflow-x-hidden" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        @keyframes pulse-violet{0%,100%{box-shadow:0 0 18px rgba(124,58,237,0.55),0 0 36px rgba(124,58,237,0.2);transform:translateY(0px)}50%{box-shadow:0 0 32px rgba(124,58,237,0.9),0 0 64px rgba(124,58,237,0.45);transform:translateY(-5px)}}
        ...
        .btn-violet-pulse{animation:pulse-violet 2.5s ease-in-out infinite;}
        .btn-violet-pulse:hover{animation:none;transform:scale(1.06);box-shadow:0 0 40px rgba(124,58,237,1);}
      ...
      <div className="bg-gradient-to-r from-indigo-600 via-violet-600 to-pink-500 py-2.5 text-center text-white text-xs font-bold tracking-wide">
        🌍 NEXORA — 12+ modules actifs · Transfert 24 pays · PayLink · MLM 3 niveaux · Formations affiliées&nbsp;·&nbsp;
      ...
      <nav className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? "glass border-b border-gray-100 dark:border-white/10 shadow-sm" : "bg-white/0 dark:bg-transparent"}`}>
        <div className="max-w-7xl mx-auto px-5 md:px-8 h-[68px] flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-7 text-[13.5px] font-semibold text-gray-500 dark:text-gray-400">
            {[["features","Modules"],["mlm","MLM & Gains"],["paylink","PayLink"],["transfert","Transfert"],["roadmap","Roadmap"],["faq","FAQ"],["download","📱 App"]].map(([id,label]) => (
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <button onClick={() => navigate("/login")} className="flex items-center gap-1.5 text-[13.5px] font-bold bg-gray-950 dark:bg-indigo-600 text-white px-5 py-2.5 rounded-xl hover:bg-indigo-600 dark:hover:bg-indigo-500 transition-all duration-300 shadow-sm">Créer un compte <ArrowRight className="w-3.5 h-3.5" /></button>
          </div>
        ...
        {menuOpen && (
          <div className="md:hidden glass border-b border-gray-100 dark:border-white/10 px-6 pb-5 pt-2 flex flex-col gap-1">
            {[["features","Tous les modules"],["mlm","MLM & Gains"],["paylink","PayLink"],["transfert","Transfert Africa"],["roadmap","Roadmap"],["faq","FAQ"],["download","📱 App"]].map(([id,label]) => (
              <button key={id} onClick={() => scrollTo(id)} className="text-left px-3 py-2.5 text-sm font-semibold text-gray-700 dark:text-gray-200 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-xl transition-colors">{label}</button>
            ))}
        ...
        <div className="relative max-w-7xl mx-auto px-5 md:px-8 pt-14 pb-20 md:pt-20 md:pb-28">
          <div className="flex justify-center mb-7">
            <div className="anim-fadeup inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white dark:bg-white/10 border border-indigo-100 dark:border-indigo-700 shadow-md text-xs font-black text-indigo-600 dark:text-indigo-300 uppercase tracking-wider">
              ...
              <span className="px-2 py-0.5 bg-indigo-600 text-white rounded-full text-[10px] font-black">12+ MODULES</span>
            </div>
          </div>
          <div className="text-center">
            <h1 className="anim-fadeup text-[3.2rem] md:text-[5.5rem] font-black tracking-tight leading-[1.04] mb-7 text-gray-950 dark:text-white" style={{ animationDelay: ".08s" }}>
              Gérez, encaissez,<br /><span className="grad-text">parrainez & gagnez</span><br /><span>en Afrique</span>
            </h1>
            <p className="anim-fadeup text-gray-500 dark:text-gray-300 text-lg md:text-xl max-w-2xl mx-auto mb-10 leading-relaxed font-normal" style={{ animationDelay: ".18s" }}>
              NEXORA réunit gestion financière, facturation, boutique e-commerce, PayLink, transfert d'argent, réseau MLM 3 niveaux, formations affiliées, coffre-fort et plus — dans une seule app conçue pour l'Afrique.
            </p>
            <div className="anim-fadeup flex flex-col sm:flex-row items-center justify-center gap-4 mb-10" style={{ animationDelay: ".28s" }}>
              <button onClick={() => navigate("/login")} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-gradient-to-br from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white font-bold text-[15px] px-8 py-4 rounded-2xl shadow-xl shadow-indigo-500/30 transition-all hover:scale-105 active:scale-95">Commencer gratuitement <ArrowRight className="w-4 h-4" /></button>
              <button onClick={() => setVideoOpen(true)} className="w-full sm:w-auto flex items-center justify-center gap-2.5 text-gray-700 dark:text-gray-200 font-semibold text-[15px] px-7 py-4 rounded-2xl border border-gray-200 dark:border-white/20 bg-white dark:bg-white/5 shadow-sm hover:shadow-md hover:border-gray-300 dark:hover:border-white/40 transition-all">
                <div className="w-7 h-7 rounded-full bg-indigo-600 flex items-center justify-center flex-shrink-0"><Play className="w-3 h-3 text-white fill-white" /></div>
                Voir la démo
              </button>
            </div>
            <div className="anim-fadeup flex flex-wrap items-center justify-center gap-x-7 gap-y-2 text-sm text-gray-400 font-medium" style={{ animationDelay: ".38s" }}>
              ...
              <span className="flex items-center gap-1.5"><Network className="w-4 h-4 text-violet-500" /> MLM 3 niveaux</span>
              <span className="hidden sm:block w-1 h-1 rounded-full bg-gray-200 dark:bg-gray-600" />
              <span className="flex items-center gap-1.5"><Globe className="w-4 h-4 text-indigo-500" /> 24 pays Transfert</span>
              <span className="hidden sm:block w-1 h-1 rounded-full bg-gray-200 dark:bg-gray-600" />
          ...
          <div className="anim-fadeup mt-16 grid grid-cols-2 md:grid-cols-4 gap-3 max-w-3xl mx-auto" style={{ animationDelay: ".5s" }}>
            {[
              ...
              { icon: Network, label: "Réseau MLM", value: "3 niveaux actifs", color: "#8b5cf6" },
              { icon: CreditCard, label: "PayLink", value: "24 pays actifs", color: "#06b6d4" },
              { icon: GraduationCap, label: "Formations", value: "30% commission", color: "#f59e0b" },
      ...
      {/* MLM SECTION */}
      <section id="mlm" className="bg-gray-950 py-20 md:py-28">
        <div className="max-w-7xl mx-auto px-5 md:px-8">
          <div className="text-center mb-14">
            <SectionBadge text="Réseau MLM" color="#8b5cf6" />
            <h2 className="text-4xl md:text-5xl font-black text-white mt-5 mb-4 leading-tight">Gagnez en dormant.<br /><span style={{ color: "#a78bfa" }}>3 niveaux de commissions.</span></h2>
            <p className="text-gray-400 text-lg max-w-xl mx-auto">Parrainez des membres, construisez votre réseau et recevez des commissions automatiques sur leurs abonnements et achats.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            {[
              { level: 1, pct: "30%", color: "#f59e0b", glow: "#f59e0b40", label: "Vos filleuls directs", desc: "Chaque abonnement Premium de vos filleuls directs vous rapporte automatiquement 30% de commission.", icon: "🥇" },
              { level: 2, pct: "10%", color: "#10b981", glow: "#10b98140", label: "Filleuls de vos filleuls", desc: "Le deuxième niveau de votre réseau vous rapporte 10% sur leurs abonnements, sans aucun effort supplémentaire.", icon: "🥈" },
              { level: 3, pct: "5%",  color: "#6366f1", glow: "#6366f140", label: "Troisième niveau", desc: "Jusqu'au 3ème niveau de profondeur, vous continuez à percevoir 5% de commission sur chaque abonnement.", icon: "🥉" },
            ].map((lvl) => (
              <div key={lvl.level} className="relative rounded-3xl border border-white/10 p-8 text-center overflow-hidden" style={{ background: `radial-gradient(ellipse at top, ${lvl.glow} 0%, #111827 60%)` }}>
                <div className="text-4xl mb-4">{lvl.icon}</div>
                <div className="text-5xl font-black mb-2" style={{ color: lvl.color }}>{lvl.pct}</div>
                <div className="text-white font-black text-lg mb-1">Niveau {lvl.level}</div>
                <div className="text-sm font-semibold mb-4" style={{ color: lvl.color }}>{lvl.label}</div>
                <p className="text-gray-400 text-sm leading-relaxed">{lvl.desc}</p>
              </div>
            ))}
          </div>
          <div className="bg-white/5 border border-white/10 rounded-3xl p-8 md:p-10 flex flex-col md:flex-row items-center gap-8">
            <div className="flex-1">
              <h3 className="text-2xl font-black text-white mb-3">Comment ça fonctionne ?</h3>
              <div className="space-y-4">
                {[
                  { step: "1", text: "Créez votre compte NEXORA gratuitement et obtenez votre lien de parrainage unique." },
                  { step: "2", text: "Partagez votre lien à vos contacts, sur WhatsApp, réseaux sociaux ou groupes." },
                  { step: "3", text: "Vos filleuls s'inscrivent et prennent un abonnement Premium." },
                  { step: "4", text: "Vous recevez automatiquement vos commissions sur 3 niveaux, retirables via Mobile Money." },
                ].map((s) => (
                  <div key={s.step} className="flex items-start gap-4">
                    <div className="w-8 h-8 rounded-full bg-violet-500 flex items-center justify-center text-white font-black text-sm flex-shrink-0">{s.step}</div>
            ...
            <div className="flex-shrink-0 w-full md:w-64">
              <div className="bg-gray-800 rounded-2xl p-6 border border-white/10">
                <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mb-4">Exemple de gains mensuels</p>
                {[
                  { label: "10 filleuls Niveau 1", gain: "3 000 FCFA", color: "#f59e0b" },
                  { label: "50 filleuls Niveau 2", gain: "5 000 FCFA", color: "#10b981" },
                  { label: "200 filleuls Niveau 3", gain: "10 000 FCFA", color: "#6366f1" },
                ].map((r) => (
                  <div key={r.label} className="flex items-center justify-between py-3 border-b border-white/5 last:border-0">
                    <span className="text-xs text-gray-400">{r.label}</span>
                    <span className="text-xs font-black" style={{ color: r.color }}>{r.gain}</span>
                  </div>
                ))}
                <div className="mt-4 pt-4 border-t border-white/10 flex items-center justify-between">
                  <span className="text-sm font-black text-white">Total potentiel</span>
                  <span className="text-sm font-black text-emerald-400">18 000+ FCFA/mois</span>
                </div>
              </div>
            </div>
          </div>
          <div className="text-center mt-10">
            <button onClick={() => navigate("/login")} className="inline-flex items-center gap-2 bg-violet-600 hover:bg-violet-500 text-white font-bold px-8 py-4 rounded-2xl shadow-lg shadow-violet-500/30 hover:scale-105 transition-all">
              Démarrer mon réseau MLM <ArrowRight className="w-4 h-4" />
            </button>
          </div>
        </div>
      </section>
      ...
      <section id="paylink" className="max-w-7xl mx-auto px-5 md:px-8 py-20 md:py-28">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <div>
            <SectionBadge text="Nexora PayLink" color="#06b6d4" />
            ...
            <button onClick={() => navigate("/login")} className="inline-flex items-center gap-2 bg-cyan-500 hover:bg-cyan-400 text-white font-bold px-7 py-3.5 rounded-2xl shadow-lg shadow-cyan-500/30 hover:scale-105 transition-all">
        ...
        <div className="max-w-7xl mx-auto px-5 md:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            <div>
              ...
              <button onClick={() => navigate("/login")} className="inline-flex items-center gap-2 bg-sky-500 hover:bg-sky-400 text-white font-bold px-7 py-3.5 rounded-2xl shadow-lg shadow-sky-500/30 hover:scale-105 transition-all">Accéder au Transfert <ArrowRight className="w-4 h-4" /></button>
            </div>
            <div className="relative">
              <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-3xl p-7 border border-white/10 shadow-2xl">
      ...
      <section id="avis" className="max-w-7xl mx-auto px-5 md:px-8 py-20 md:py-28">
          ...
          {reviews.map((r,i)=>(
            <div key={i} className="card-lift bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-3xl p-7">
              ...
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-full bg-gradient-to-br from-indigo-500 to-pink-500 flex items-center justify-center text-white font-black text-base flex-shrink-0 shadow-md">{r.name.charAt(0)}</div>
        ...
        <div className="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-950/50 dark:to-purple-950/40 border border-indigo-100 dark:border-indigo-800 rounded-3xl p-8 md:p-10 max-w-2xl mx-auto">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-2xl bg-indigo-600 flex items-center justify-center flex-shrink-0"><MessageSquare className="w-5 h-5 text-white" /></div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={reviewForm.name} onChange={(e)=>setReviewForm((p)=>({...p,name:e.target.value}))} placeholder="Ex : Kouassi Jean" className="w-full px-4 py-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 text-gray-900 dark:text-gray-100 placeholder-gray-400 rounded-xl text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-900 transition-all" />
              </div>
              <div>
            ...
            <button onClick={submitReview} disabled={reviewForm.name.trim().split(/\s+/).length<2||!reviewForm.name.trim().split(/\s+/)[1]||!reviewForm.text} className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold rounded-xl transition-colors flex items-center justify-center gap-2">
      ...
      <section id="faq" className="max-w-4xl mx-auto px-5 md:px-8 py-20 md:py-28">
        ...
        <div className="space-y-3">
          {[
            ...
            { q: "Comment fonctionne le système MLM ?", a: "Parrainez des membres avec votre lien unique. Quand vos filleuls prennent un abonnement Premium, vous recevez automatiquement 30% de commission (Niveau 1), 10% sur leurs filleuls (Niveau 2) et 5% sur le troisième niveau. Les commissions sont retirables via Mobile Money." },
            ...
            { q: "Comment gagner avec les Formations & Affiliation ?", a: "Achetez une formation, obtenez votre lien affilié unique, et gagnez 30% de commission sur chaque vente générée via votre lien. Les stats de clics et ventes sont disponibles en temps réel dans votre dashboard." },
      ...
      <section id="download" className="max-w-7xl mx-auto px-5 md:px-8 py-20 md:py-28">
        <div className="relative bg-gray-950 rounded-3xl overflow-hidden">
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute -top-24 -left-24 w-96 h-96 rounded-full bg-violet-500/10 blur-3xl" />
          ...
          <div className="relative flex flex-col md:flex-row items-center gap-12 px-8 md:px-16 py-16 md:py-20">
            <div className="flex-1 text-center md:text-left">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-violet-500/10 border border-violet-500/30 text-xs font-black text-violet-400 mb-6 uppercase tracking-widest"><span>📱</span> Application Mobile</div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
                <a href="/nexora.apk" download="nexora.apk" className="btn-violet-pulse group inline-flex items-center gap-3 text-white font-black px-8 py-4 rounded-2xl text-base active:scale-95" style={{background:"linear-gradient(135deg,#7c3aed 0%,#6d28d9 50%,#8b5cf6 100%)"}}>
            ...
            <div className="flex-shrink-0 flex items-center justify-center">
              <div className="relative">
                <div className="absolute inset-0 rounded-full bg-violet-500/20 blur-3xl scale-150" />
                <div className="relative w-48 h-80 bg-gray-800 rounded-[2.5rem] border-4 border-gray-700 shadow-2xl flex flex-col overflow-hidden">
                  ...
                  <div className="flex-1 bg-[#0a0e27] flex flex-col items-center justify-center gap-3 p-4">
                    ...
                    <div className="w-full bg-violet-500/20 border border-violet-500/30 rounded-xl p-2 mt-2">
                      <p className="text-violet-400 text-[10px] font-bold text-center">✓ Application installée</p>
                    </div>
                  </div>
      ...
      <section className="max-w-7xl mx-auto px-5 md:px-8 pb-20">
        <div className="relative bg-gray-950 rounded-3xl overflow-hidden py-16 md:py-24 px-8 md:px-16 text-center">
          ...
          <div className="relative">
            ...
            <button onClick={()=>navigate("/login")} className="inline-flex items-center gap-2 bg-white text-gray-950 font-black px-10 py-4 rounded-2xl text-lg hover:bg-gray-100 transition-all shadow-2xl hover:scale-105 active:scale-95">Commencer maintenant <ArrowRight className="w-5 h-5" /></button>
        ...
        <div className="max-w-7xl mx-auto px-5 md:px-8 pt-14 pb-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-12">
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">{["Finances","Facturation","Boutique","PayLink","Transfert","MLM & Réseau","Formations","Academy","Coffre-Fort","Immobilier","Contacts WhatsApp","Abonnements & Liens"].map((l)=><button key={l} onClick={()=>navigate("/login")} className="text-sm text-gray-500 hover:text-white text-left transition-colors">{l}</button>)}</div>
            </div>
            <div>
