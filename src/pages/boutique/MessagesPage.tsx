...
import {
  ...
  Package, ShoppingCart, Clock, CheckCheck, Search, Filter,
  RefreshCw, AlertCircle, User, Store
} from "lucide-react";
...
export default function MessagesPage() {
  ...
  // ── Charger boutique de l'utilisateur ──
  useEffect(() => {
    (async () => {
  ...
  const loadDiscussions = async () => {
    ...
    const { data } = await (supabase as any)
      ...
      .select(`
        *,
        produit:produit_id ( nom, photos ),
        commande:commande_id ( numero )
      `)
    ...
    if (data) {
      ...
      const withLast = await Promise.all(
        (data as Discussion[]).map(async (d) => {
          ...
          return {
            ...d,
            dernierMessage: lastMsg?.contenu || (lastMsg?.fichier_type === "image" ? "📷 Image" : lastMsg?.fichier_type === "video" ? "🎥 Vidéo" : ""),
          };
        })
      );
  ...
  // ── Charger messages d'une discussion ──
  const loadMessages = async (discId: string) => {
    ...
    // Marquer comme lu côté vendeur
    await (supabase as any)
      .from("discussions")
      .update({ lu_vendeur: true })
      .eq("id", discId);
    await (supabase as any)
      .from("messages_discussion")
      .update({ lu: true })
    ...
    setDiscussions(prev => prev.map(d => d.id === discId ? { ...d, lu_vendeur: true } : d));
    setTimeout(() => bottomRef.current?.scrollIntoView({ behavior: "smooth" }), 100);
  };

  // ── Poll messages toutes les 5s ──
  useEffect(() => {
    if (!activeId) return;
  ...
  const uploadMedia = async (file: File): Promise<{ url: string; type: string; nom: string } | null> => {
    setUploading(true);
  ...
  const sendMessage = async () => {
    if (!activeId || (sending && !uploadFile && !newMsg.trim())) return;
    setSending(true);
  ...
  return (
    <div className="flex h-[calc(100vh-80px)] bg-gray-50 rounded-2xl overflow-hidden border border-gray-200 shadow-sm">
      ...
      <div className={`${mobileView === "chat" ? "hidden" : "flex"} md:flex flex-col w-full md:w-80 lg:w-96 border-r border-gray-200 bg-white flex-shrink-0`}>
        ...
        <div className="p-4 border-b border-gray-100">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              {nonLus > 0 && (
                <span className="bg-[#305CDE] text-white text-xs font-bold px-2 py-0.5 rounded-full">{nonLus}</span>
              )}
            </div>
            <button onClick={loadDiscussions} className="p-1.5 rounded-lg hover:bg-gray-100 transition-colors">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#305CDE]/30"
            />
          </div>
        </div>
          ...
          {loading ? (
            ...
            filteredDiscussions.map(d => (
              <button
                ...
                className={`w-full p-4 text-left hover:bg-gray-50 transition-colors border-b border-gray-50 ${activeId === d.id ? "bg-[#305CDE]/5 border-l-4 border-l-[#305CDE]" : ""}`}
              >
                <div className="flex items-start gap-3">
                  {/* Avatar */}
                  <div className={`w-10 h-10 rounded-2xl flex items-center justify-center text-white font-black text-sm flex-shrink-0 ${activeId === d.id ? "bg-[#305CDE]" : "bg-gray-300"}`}>
                    {d.acheteur_nom.charAt(0).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className={`text-sm font-bold truncate ${!d.lu_vendeur ? "text-gray-900" : "text-gray-600"}`}>
                        {d.acheteur_nom}
                      </p>
                      <span className="text-[10px] text-gray-400 flex-shrink-0">{timeAgo(d.updated_at)}</span>
                    </div>
                    {/* Produit / Commande */}
                    ...
                    {d.dernierMessage && (
                      <p className={`text-xs mt-1 truncate ${!d.lu_vendeur ? "text-gray-700 font-semibold" : "text-gray-400"}`}>
                        {d.dernierMessage}
                      </p>
                    )}
                  </div>
                  {!d.lu_vendeur && (
                    <div className="w-2.5 h-2.5 bg-[#305CDE] rounded-full flex-shrink-0 mt-1" />
                  )}
                </div>
              </button>
            ))
      ...
      {/* ── Panneau droit : chat ── */}
      <div className={`${mobileView === "list" ? "hidden" : "flex"} md:flex flex-col flex-1 bg-white`}>
        {!activeDiscussion ? (
          <div className="flex flex-col items-center justify-center h-full text-center px-8">
            ...
            <h2 className="text-xl font-black text-gray-800">Vos messages</h2>
            <p className="text-sm text-gray-500 mt-2 max-w-xs">
              Sélectionnez une discussion à gauche pour voir les messages de vos acheteurs.
            </p>
          </div>
        ) : (
          <>
            ...
            <div className="flex items-center gap-3 p-4 border-b border-gray-100 bg-white">
              ...
              <div className="w-10 h-10 bg-[#305CDE] rounded-2xl flex items-center justify-center text-white font-black text-sm">
                {activeDiscussion.acheteur_nom.charAt(0).toUpperCase()}
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-bold text-gray-900 text-sm">{activeDiscussion.acheteur_nom}</p>
              ...
              <span className={`text-xs px-2.5 py-1 rounded-full font-semibold ${
                activeDiscussion.statut === "ouvert" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"
              }`}>
                ...
                messages.map(msg => {
                  ...
                  return (
                    <div key={msg.id} className={`flex ${isVendeur ? "justify-end" : "justify-start"}`}>
                      ...
                      <div className={`max-w-[75%] space-y-1 ${isVendeur ? "items-end" : "items-start"} flex flex-col`}>
                        {/* Fichier image */}
                        ...
                        {msg.fichier_url && msg.fichier_type === "video" && (
                          <div className={`rounded-2xl overflow-hidden shadow-sm ${isVendeur ? "rounded-br-sm" : "rounded-bl-sm"}`}>
                            <video
                              src={msg.fichier_url}
                              controls
                              className="max-w-[240px] max-h-[200px] rounded-2xl"
                            />
                          </div>
                        )}
                        {/* Texte */}
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          {isVendeur && <CheckCheck className={`w-3 h-3 ${msg.lu ? "text-[#305CDE]" : "text-gray-300"}`} />}
                        </div>
                      </div>
                    </div>
                  );
            ...
            {/* Preview fichier à envoyer */}
            {uploadFile && (
              <div className="px-4 py-2 bg-blue-50 border-t border-blue-100 flex items-center gap-3">
                {uploadFile.type.startsWith("image") ? (
                  <img src={URL.createObjectURL(uploadFile)} className="w-12 h-12 rounded-lg object-cover" />
                ) : (
              ...
              <div className="flex items-end gap-2">
                ...
                <div className="flex-1 relative">
                  <textarea
                    ...
                    rows={1}
                    className="w-full px-4 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#305CDE]/30 resize-none max-h-32"
