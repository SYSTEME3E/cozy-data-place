/**
 * BIEN-ÊTRE YUPI — Page publique (sans compte requis)
 * Accessible via : /bien-etre-yupi
 * Commande sauvegardée en DB · Aucun compte nécessaire
 */
...
import {
  ...
  MapPin, User, Package, Info, MessageCircle, Baby, Activity,
  CheckCircle, LogIn, LayoutDashboard
} from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { getNexoraUser } from "@/lib/nexora-auth";

const VENDEUR_WHATSAPP      = "2290151762341";
const VENDEUR_DISPLAY       = "+229 01 51 76 23 41";
const LOGO_URL              = "https://i.postimg.cc/bYSB8r5L/Yupi-logo-1536x1058.png";

/* ═══════════════════════════════════════════════════════════
   DONNÉES PRODUITS (identiques à YupiGlobalShopPage)
══════════════════════════════════════════════════════════════ */
const PRODUITS = [
  { id:1,  nom:"ALKA PLUS",             prix:18750, img:"https://i.postimg.cc/zfXPnf3Z/1769666515980.jpg",        couleur:"from-blue-500 to-cyan-400",    badge:"Équilibre pH",      badgeColor:"bg-blue-500",
    ...
    combinaisons:[{maladie:"Reflux & Gastrite chronique",produits:["ALKA PLUS","PILON CARE"],description:"Association puissante pour neutraliser l'acidité et cicatriser la muqueuse digestive."},{maladie:"Arthrite & Douleurs articulaires",produits:["ALKA PLUS","PAIN AND COLD BALM","GOLDEN PAIN OIL"],description:"Triple action anti-inflammatoire et alcalinisante pour soulager durablement les articulations."},{maladie:"Calculs rénaux",produits:["ALKA PLUS","DÉTOX (60 caps)"],description:"Alcalinisation + détoxification pour dissoudre les calculs et nettoyer les reins."}]},
  { id:2,  nom:"DÉTOX (60 caps)",        prix:18750, img:"https://i.postimg.cc/mZcJXhw3/detox-health30.jpg",       couleur:"from-green-500 to-emerald-400",badge:"Détox Profonde",   badgeColor:"bg-green-500",
    ...
    combinaisons:[{maladie:"Nettoyage hépatique complet",produits:["DÉTOX (60 caps)","SEA BUCKTHORN JUICE"],description:"Combinaison idéale pour régénérer le foie et protéger les cellules hépatiques."},{maladie:"Perte de poids saine",produits:["DÉTOX (60 caps)","ALKA PLUS"],description:"Détoxification profonde + alcalinisation pour une perte de poids durable."},{maladie:"Foie gras (stéatose hépatique)",produits:["DÉTOX (60 caps)","SEA BUCKTHORN JUICE","YUPI DRINK (CAFÉ)"],description:"Triple action pour nettoyer le foie, réduire les graisses et régénérer les hépatocytes."}]},
  { id:3,  nom:"IMMUNO BOOST (60 caps)", prix:18750, img:"https://i.imgur.com/eHXzZbx.jpeg",                        couleur:"from-violet-500 to-purple-400",badge:"Immunité Max",     badgeColor:"bg-violet-500",
    ...
    combinaisons:[{maladie:"VIH & Infections virales sévères",produits:["IMMUNO BOOST (60 caps)","SEA BUCKTHORN JUICE","YUPI DRINK (CAFÉ)"],description:"Synergie immunologique maximale pour booster les défenses et régénérer les cellules immunitaires."},{maladie:"Hépatites virales",produits:["IMMUNO BOOST (60 caps)","DÉTOX (60 caps)"],description:"Action antivirale et hépatoprotectrice pour restaurer totalement la santé du foie."},{maladie:"Lupus & Maladies auto-immunes",produits:["IMMUNO BOOST (60 caps)","SEA BUCKTHORN JUICE","ALKA PLUS"],description:"Régulation immunitaire + antioxydants puissants + équilibre pH pour calmer les auto-attaques."}]},
  { id:4,  nom:"PAIN AND COLD BALM",     prix:2500,  img:"https://i.imgur.com/DtFNIaI.jpeg",                        couleur:"from-red-500 to-rose-400",     badge:"Anti-douleur",     badgeColor:"bg-red-500",
    ...
    combinaisons:[{maladie:"Arthrite sévère",produits:["PAIN AND COLD BALM","GOLDEN PAIN OIL","ALKA PLUS"],description:"Application externe + ingestion interne pour un soulagement total des douleurs articulaires."},{maladie:"Douleurs de règles intenses",produits:["PAIN AND COLD BALM","WOMEN CARE"],description:"Soulagement topique immédiat + régulation hormonale pour des règles moins douloureuses."}]},
  { id:5,  nom:"GOLDEN PAIN OIL",        prix:8750,  img:"https://i.postimg.cc/j5ST3zZc/IMG-20260201-WA0009.jpg",   couleur:"from-yellow-500 to-amber-400", badge:"Huile Thérap.",    badgeColor:"bg-yellow-500",
    ...
    combinaisons:[{maladie:"Sciatique & Névralgie",produits:["GOLDEN PAIN OIL","PAIN AND COLD BALM","SEA BUCKTHORN JUICE"],description:"Double action locale puissante + protection nerveuse par les antioxydants."},{maladie:"Arthrose avancée",produits:["GOLDEN PAIN OIL","ALKA PLUS","SEA BUCKTHORN JUICE"],description:"Réduction de l'inflammation, régénération du cartilage et protection articulaire durable."}]},
  { id:6,  nom:"COSTI AWAY",             prix:13125, img:"https://i.postimg.cc/JhQXVNNK/58516261-Nj-Iw-LTgy-Ny1k-Yz-Q4OWRj-Ym.webp", couleur:"from-teal-500 to-cyan-400", badge:"Anti-parasites",  badgeColor:"bg-teal-500",
    ...
    combinaisons:[{maladie:"Parasites & Toxines",produits:["COSTI AWAY","DÉTOX (60 caps)"],description:"Éradication des parasites + élimination des toxines laissées par ces parasites."},{maladie:"Côlon irritable",produits:["COSTI AWAY","ALKA PLUS","PILON CARE"],description:"Assainissement du côlon + équilibre pH + cicatrisation intestinale pour retrouver un transit sain."}]},
  { id:7,  nom:"WOMEN CARE",             prix:15625, img:"https://i.postimg.cc/sfJQtyvB/Whats-App-Image-2025-01-18-at-1-22-20-PM-4.jpg", couleur:"from-pink-500 to-rose-400", badge:"Santé Femme",     badgeColor:"bg-pink-500",
    ...
    combinaisons:[{maladie:"Infertilité féminine",produits:["WOMEN CARE","SEA BUCKTHORN JUICE","IMMUNO BOOST (60 caps)"],description:"Rééquilibrage hormonal + qualité des ovules + immunité optimale pour favoriser la conception."},{maladie:"SOPK & Irrégularités hormonales",produits:["WOMEN CARE","DÉTOX (60 caps)","ALKA PLUS"],description:"Détox + alcalinisation + régulation hormonale pour vaincre le SOPK durablement."},{maladie:"Fibromes & Kystes ovariens",produits:["WOMEN CARE","DÉTOX (60 caps)","SEA BUCKTHORN JUICE"],description:"Action combinée pour dissoudre les fibromes, réduire les kystes et restaurer l'équilibre utérin."},{maladie:"Ménopause difficile",produits:["WOMEN CARE","YUPI DRINK (CAFÉ)","SEA BUCKTHORN JUICE"],description:"Équilibre hormonal naturel + antioxydants pour traverser la ménopause sans souffrir."},{maladie:"Règles douloureuses & abondantes",produits:["WOMEN CARE","PAIN AND COLD BALM"],description:"Régulation hormonale profonde + soulagement topique immédiat des douleurs pelviennes."}]},
  { id:8,  nom:"MEN POWER OIL",          prix:6250,  img:"https://i.postimg.cc/zf1qzbQK/men-power-oil-1.jpg",       couleur:"from-blue-600 to-indigo-500",  badge:"Huile Virilité",  badgeColor:"bg-blue-600",
    ...
    combinaisons:[{maladie:"Performance masculine complète",produits:["MEN POWER OIL","MEN POWER MALT"],description:"Action externe (massage) + interne (nutrition) pour une virilité optimale et durable."},{maladie:"Dysfonction érectile",produits:["MEN POWER OIL","MEN POWER MALT","YUPI DRINK (CAFÉ)"],description:"Triple synergie pour restaurer une érection ferme, durable et une libido au maximum."}]},
  { id:9,  nom:"SEA BUCKTHORN JUICE",    prix:18750, img:"https://i.imgur.com/dCulCdA.jpeg",                        couleur:"from-orange-500 to-yellow-400",badge:"Antioxydant",     badgeColor:"bg-orange-500",
    ...
    combinaisons:[{maladie:"Cholestérol & Santé cardiaque",produits:["SEA BUCKTHORN JUICE","DÉTOX (60 caps)","ALKA PLUS"],description:"Protection cardiovasculaire globale : détox, alcalinisation et antioxydants naturels."},{maladie:"Anti-âge & Beauté peau",produits:["SEA BUCKTHORN JUICE","YUPI DRINK (CAFÉ)"],description:"Régénération cellulaire complète pour une peau lumineuse et un corps rajeuni."},{maladie:"Fertilité (homme & femme)",produits:["SEA BUCKTHORN JUICE","WOMEN CARE","IMMUNO BOOST (60 caps)"],description:"Antioxydants + hormones équilibrées + immunité renforcée pour maximiser les chances de conception."}]},
  { id:10, nom:"DIABO CARE",             prix:15625, img:"https://i.postimg.cc/sXsxjQKz/IMG-20260201-WA0003.jpg",   couleur:"from-purple-500 to-violet-400",badge:"Anti-diabète",    badgeColor:"bg-purple-500",
    ...
    combinaisons:[{maladie:"Diabète type 2 complet",produits:["DIABO CARE","DIABO CARE SPRAY","SEA BUCKTHORN JUICE"],description:"Régulation glycémique interne + soin des plaies + protection cellulaire antioxydante."},{maladie:"Diabète + Hypertension",produits:["DIABO CARE","ALKA PLUS","SEA BUCKTHORN JUICE"],description:"Contrôle du sucre + équilibre pH + protection cardio pour traiter les deux ensemble."}]},
  { id:11, nom:"DIABO CARE SPRAY",       prix:8750,  img:"https://i.postimg.cc/pT6pRqpQ/dd11f0fc-4b49-4537-aba5-28b18d5f9d34.jpg", couleur:"from-purple-400 to-pink-400", badge:"Plaies Diabète", badgeColor:"bg-purple-400",
    ...
    combinaisons:[{maladie:"Plaies & Ulcères diabétiques (éviter amputation)",produits:["DIABO CARE SPRAY","DIABO CARE","SEA BUCKTHORN JUICE"],description:"Traitement interne + externe + antioxydants pour soigner les plaies et éviter l'amputation."}]},
  { id:12, nom:"DÉTOX HEATH (30 caps)",  prix:9325,  img:"https://i.postimg.cc/mZcJXhw3/detox-health30.jpg",        couleur:"from-green-400 to-teal-400",   badge:"Mini-Détox",       badgeColor:"bg-green-400",
    ...
    combinaisons:[{maladie:"Cure d'entretien mensuelle",produits:["DÉTOX HEATH (30 caps)","ALKA PLUS"],description:"Cure mensuelle de détox légère + équilibre pH pour maintenir l'organisme en santé optimale."}]},
  { id:13, nom:"PILON CARE",             prix:15625, img:"https://i.postimg.cc/tgg2VDwm/Pilon-care.png",            couleur:"from-red-400 to-orange-400",   badge:"Hémorroïdes",      badgeColor:"bg-red-400",
    ...
    combinaisons:[{maladie:"Hémorroïdes sévères + Constipation",produits:["PILON CARE","ALKA PLUS","DÉTOX (60 caps)"],description:"Traitement des hémorroïdes + équilibre pH + nettoyage du côlon pour une guérison complète."}]},
  { id:14, nom:"MEN POWER MALT",         prix:18750, img:"https://i.imgur.com/psVzO57.jpeg",                        couleur:"from-blue-700 to-blue-500",    badge:"Virilité Max",    badgeColor:"bg-blue-700",
    ...
    combinaisons:[{maladie:"Infertilité masculine",produits:["MEN POWER MALT","SEA BUCKTHORN JUICE","IMMUNO BOOST (60 caps)"],description:"Amélioration de la qualité spermatique + protection antioxydante + immunité renforcée."},{maladie:"Dysfonction érectile sévère",produits:["MEN POWER MALT","MEN POWER OIL","YUPI DRINK (CAFÉ)"],description:"Action interne puissante + massage local + énergie globale pour restaurer une érection totale."},{maladie:"Éjaculation précoce",produits:["MEN POWER MALT","MEN POWER OIL"],description:"Régulation du système nerveux + amélioration du contrôle musculaire pour une meilleure endurance."}]},
  { id:15, nom:"IMMUNO BOOST (30 caps)", prix:9375,  img:"https://i.imgur.com/eHXzZbx.jpeg",                        couleur:"from-violet-400 to-blue-400",  badge:"Immunité 30j",    badgeColor:"bg-violet-400",
    ...
    combinaisons:[{maladie:"Grippe & Infections ORL",produits:["IMMUNO BOOST (30 caps)","DENTAL DROP"],description:"Immunité renforcée + soin des voies ORL pour une guérison rapide des infections respiratoires."},{maladie:"Convalescence rapide",produits:["IMMUNO BOOST (30 caps)","YUPI DRINK (CAFÉ)","SEA BUCKTHORN JUICE"],description:"Récupération accélérée grâce à l'énergie, l'immunité et les antioxydants réunis."}]},
  { id:16, nom:"DENTAL DROP",            prix:2500,  img:"https://i.postimg.cc/jqGF1Hjf/IMG-20260201-WA0013.jpg",   couleur:"from-sky-400 to-blue-300",    badge:"Santé Dentaire",  badgeColor:"bg-sky-400",
    ...
    combinaisons:[{maladie:"Infections buccales sévères & Abcès",produits:["DENTAL DROP","IMMUNO BOOST (30 caps)","SEA BUCKTHORN JUICE"],description:"Soin buccal local + immunité renforcée + antioxydants pour une guérison complète et durable."}]},
  { id:17, nom:"YUPI DRINK (CAFÉ)",      prix:12500, img:"https://i.postimg.cc/FsR5NgsZ/yupi-drink-coffee-1.png",   couleur:"from-amber-700 to-yellow-500",badge:"Énergie & Bien-être",badgeColor:"bg-amber-700",
    ...
    combinaisons:[{maladie:"Burn-out & Fatigue extrême",produits:["YUPI DRINK (CAFÉ)","SEA BUCKTHORN JUICE","IMMUNO BOOST (60 caps)"],description:"Énergie immédiate + protection antioxydante + immunité pour vaincre la fatigue profonde."},{maladie:"Prostate & Santé masculine",produits:["YUPI DRINK (CAFÉ)","MEN POWER MALT","SEA BUCKTHORN JUICE"],description:"Triple soutien pour la prostate, la virilité et la santé cardiovasculaire masculine."},{maladie:"Mémoire & Concentration",produits:["YUPI DRINK (CAFÉ)","SEA BUCKTHORN JUICE"],description:"Boost cognitif naturel + protection neuronale pour rester alerte, concentré et performant."}]},
];
...
type View = "shop" | "detail" | "cart" | "checkout" | "success";

/* ═══════════════════════════════════════════════════════════
   LAYOUT PUBLIC — header minimaliste avec logo YUPI
══════════════════════════════════════════════════════════════ */
function YupiLayout({ children, cartCount, onCartClick }: { children: React.ReactNode; cartCount: number; onCartClick: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      {/* ── Bandeau promo ── */}
      <div className="bg-gradient-to-r from-green-600 to-emerald-600 text-white text-center text-xs font-bold py-2 px-4">
        🌿 BIEN-ÊTRE YUPI · Produits 100% naturels · Livraison partout au Bénin 🇧🇯 · {VENDEUR_DISPLAY}
      </div>
      ...
      <header className="sticky top-0 z-50 bg-background/95 backdrop-blur border-b border-border/60 shadow-sm">
        <div className="max-w-5xl mx-auto flex items-center justify-between h-16 px-4">
          ...
          <button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl overflow-hidden border-2 border-green-400/40 shadow">
              <img src={LOGO_URL} alt="BIEN-ÊTRE YUPI" className="w-full h-full object-cover" />
            </div>
            <div className="text-left">
              <p className="font-black text-base text-foreground leading-none">BIEN-ÊTRE YUPI</p>
              <p className="text-[10px] text-green-500 font-semibold leading-none">Santé Naturelle 🌿</p>
            </div>
          </button>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={onCartClick}
              className="relative flex items-center gap-2 bg-gradient-to-r from-violet-500 to-blue-500 text-white font-bold px-4 py-2 rounded-xl text-sm shadow hover:opacity-90 transition-opacity">
      ...
      <footer className="border-t border-border/40 py-6 bg-muted/20">
        <div className="max-w-5xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-muted-foreground">
          <div className="flex items-center gap-2">
            <img src={LOGO_URL} alt="Logo" className="w-6 h-6 rounded-lg object-cover" />
            <span>© {new Date().getFullYear()} BIEN-ÊTRE YUPI · Tous droits réservés</span>
          </div>
          <a href={`https://wa.me/${VENDEUR_WHATSAPP}`} target="_blank" rel="noreferrer"
            className="flex items-center gap-1.5 text-green-500 font-bold hover:underline">
            <MessageCircle className="w-3.5 h-3.5" /> {VENDEUR_DISPLAY}
          </a>
        </div>
      </footer>
    </div>
...
/* ═══════════════════════════════════════════════════════════
   PAGE PRINCIPALE
══════════════════════════════════════════════════════════════ */
export default function YupiPublicShopPage() {
  const user = getNexoraUser();
  ...
  const goDetail      = (id: number) => { setSelectedId(id); setView("detail"); window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  return (
    <YupiLayout cartCount={cartCount} onCartClick={() => setView("cart")}>

      {/* ══════════ SUCCÈS ══════════ */}
      {view === "success" && (
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
          <div className="relative mb-6">
            <div className="w-24 h-24 rounded-3xl overflow-hidden shadow-2xl border-4 border-green-400/40">
              <img src={LOGO_URL} alt="BIEN-ÊTRE YUPI" className="w-full h-full object-cover" />
            </div>
            <div className="absolute -bottom-2 -right-2 w-9 h-9 bg-green-400 rounded-full flex items-center justify-center shadow">
          ...
          <p className="text-sm text-muted-foreground max-w-xs mb-6">Votre commande est bien enregistrée. Le vendeur vous contactera sur votre WhatsApp pour confirmer la livraison.</p>
          <div className="bg-card border border-border rounded-2xl p-5 max-w-xs w-full mb-6 text-left space-y-2.5">
            <p className="text-xs font-black text-muted-foreground uppercase tracking-widest mb-1">Récapitulatif</p>
            <div className="flex items-center gap-2 text-sm"><User className="w-4 h-4 text-violet-400 flex-shrink-0" /><span className="font-semibold">{orderForm.nom}</span></div>
            <div className="flex items-center gap-2 text-sm"><Phone className="w-4 h-4 text-green-400 flex-shrink-0" /><span>{orderForm.whatsapp}</span></div>
            <div className="flex items-center gap-2 text-sm"><MapPin className="w-4 h-4 text-red-400 flex-shrink-0" /><span>{orderForm.ville}{orderForm.adresse ? ` · ${orderForm.adresse}` : ""}</span></div>
          ...
          <button onClick={() => { setView("shop"); setOrderForm({ nom: user?.nom_prenom || "", whatsapp: "", ville: "", adresse: "", notes: "" }); setOrderRef(""); }}
            className="bg-gradient-to-r from-violet-500 to-blue-500 text-white font-black px-8 py-3.5 rounded-2xl shadow-lg hover:opacity-90">
      ...
      {view === "checkout" && (
        <div className="max-w-lg mx-auto">
          <button onClick={() => setView("cart")} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground mb-6"><ArrowLeft className="w-4 h-4" /> Retour au panier</button>
          <div className="bg-gradient-to-r from-violet-600 to-blue-600 rounded-3xl p-6 text-white mb-5">
            ...
            <p className="opacity-80 text-sm">Renseignez vos informations de livraison</p>
          </div>
          <div className="bg-card border border-border rounded-2xl p-4 mb-5">
            <p className="text-xs font-black text-muted-foreground uppercase tracking-widest mb-3">Récapitulatif</p>
            {cart.map(item => (
          ...
          <div className="bg-card border border-border rounded-2xl p-5 space-y-4">
            ...
            <button onClick={submitOrder} disabled={submitting || !orderForm.nom || !orderForm.whatsapp || !orderForm.ville}
              className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-green-500 to-emerald-500 disabled:opacity-50 text-white font-black py-4 rounded-2xl text-lg shadow-lg hover:opacity-90">
              {submitting ? <span className="animate-spin w-5 h-5 border-2 border-white/30 border-t-white rounded-full inline-block" /> : <><Send className="w-5 h-5" /> Valider ma commande</>}
            </button>
      ...
      {view === "cart" && (
        <div className="max-w-lg mx-auto">
          <button onClick={() => setView("shop")} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground mb-6"><ArrowLeft className="w-4 h-4" /> Retour à la boutique</button>
              ...
              {cart.map(item => (
                <div key={item.id} className="bg-card border border-border rounded-2xl p-4 flex items-center gap-4">
                  <img src={item.img} alt={item.nom} onClick={() => goDetail(item.id)}
                    className="w-16 h-16 rounded-xl object-cover flex-shrink-0 cursor-pointer hover:opacity-80"
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button onClick={() => updateQty(item.id, -1)} className="w-8 h-8 rounded-lg bg-muted flex items-center justify-center hover:bg-red-100 hover:text-red-500"><Minus className="w-3.5 h-3.5" /></button>
                    <span className="w-6 text-center font-black text-sm">{item.qty}</span>
                    <button onClick={() => updateQty(item.id, 1)} className="w-8 h-8 rounded-lg bg-muted flex items-center justify-center hover:bg-green-100 hover:text-green-500"><Plus className="w-3.5 h-3.5" /></button>
                    <button onClick={() => removeFromCart(item.id)} className="w-8 h-8 rounded-lg bg-red-500/10 flex items-center justify-center hover:bg-red-500/20 ml-1"><Trash2 className="w-3.5 h-3.5 text-red-500" /></button>
                  </div>
                </div>
              ))}
        ...
        <div>
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            <div className={`relative overflow-hidden rounded-3xl bg-gradient-to-br ${selected.couleur} p-1`}>
              <img src={selected.img} alt={selected.nom}
                className="w-full h-64 md:h-80 object-cover rounded-[20px]"
            ...
            <div className="flex flex-col justify-between">
              <div>
                ...
                <div className="flex items-center gap-1 mb-4">
                  ...
                  <span className="text-xs text-muted-foreground ml-2">Produit certifié</span>
                </div>
              ...
              <div className="space-y-3">
                ...
                <a href={`https://wa.me/${VENDEUR_WHATSAPP}?text=${encodeURIComponent(`Bonjour, je suis intéressé par : ${selected.nom} (${selected.prix.toLocaleString()} FCFA). Pouvez-vous me renseigner ? 🌿`)}`}
                  ...
                  className="w-full flex items-center justify-center gap-2 border-2 border-green-500 text-green-500 font-bold py-3 rounded-2xl hover:bg-green-500/10 text-sm">
          ...
          <div className="bg-card border border-border rounded-3xl p-6 mb-6">
            ...
            <ul className="space-y-2.5">
              {selected.roles.map((r, i) => <li key={i} className="flex items-start gap-3 text-sm"><div className="w-2 h-2 rounded-full bg-green-400 mt-1.5 flex-shrink-0" />{r}</li>)}
            </ul>
          </div>
          {selected.combinaisons.length > 0 && (
            <div className="space-y-4">
              <div className="flex items-center gap-2"><Zap className="w-5 h-5 text-yellow-400" /><h3 className="font-black text-lg">Protocoles de Guérison Recommandés</h3></div>
        ...
        <div>
            ...
            <div className="relative z-10 p-7 sm:p-10 text-white">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-14 h-14 rounded-2xl overflow-hidden border-2 border-white/30 shadow-lg flex-shrink-0">
                  <img src={LOGO_URL} alt="Logo BIEN-ÊTRE YUPI" className="w-full h-full object-cover" />
                </div>
              ...
              <a href={`https://wa.me/${VENDEUR_WHATSAPP}?text=${encodeURIComponent("Bonjour, j'ai une question sur les produits BIEN-ÊTRE YUPI 🌿")}`}
                target="_blank" rel="noreferrer"
                className="inline-flex items-center gap-2 bg-green-400/20 border border-green-400/40 text-white text-sm font-bold px-4 py-2.5 rounded-2xl hover:bg-green-400/30 mb-4">
                <MessageCircle className="w-4 h-4 text-green-300" /> Nous contacter : <span className="text-green-300">{VENDEUR_DISPLAY}</span>
              </a>
            ...
            {filtered.map(p => (
              <div key={p.id} className="group bg-card border border-border rounded-2xl overflow-hidden hover:border-violet-500/40 hover:shadow-xl transition-all duration-300 flex flex-col">
                ...
                <div className="p-3 flex flex-col flex-1">
                  <h3 className="font-black text-xs text-foreground mb-1 leading-tight cursor-pointer hover:text-violet-500" onClick={() => goDetail(p.id)}>{p.nom}</h3>
          ...
          <div className="mt-10 space-y-4">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {[
                { icon:"🚺", title:"Santé de la Femme",         items:["Infertilité & difficultés à concevoir","SOPK & Fibromes utérins","Règles douloureuses ou absentes","Ménopause difficile","Pertes blanches & Kystes ovariens"], bg:"from-pink-500/10 to-rose-500/10", border:"border-pink-500/20" },
                { icon:"🚹", title:"Virilité & Santé Masculine", items:["Dysfonction érectile","Infertilité masculine (sperme faible)","Éjaculation précoce","Prostate enflée","Libido & endurance sexuelle"],            bg:"from-blue-500/10 to-indigo-500/10", border:"border-blue-500/20" },
                { icon:"👶", title:"Fertilité & Conception",     items:["Difficultés à tomber enceinte","Améliorer la qualité des ovules","Améliorer la qualité du sperme","Préparer le corps à la conception","Soutien après fausse couche"],  bg:"from-emerald-500/10 to-green-500/10", border:"border-emerald-500/20" },
                { icon:"💊", title:"Maladies Chroniques",        items:["Diabète type 1 & 2","Hypertension artérielle","Cholestérol & maladies cardiaques","VIH & Hépatites virales","Maladies auto-immunes & lupus"], bg:"from-purple-500/10 to-violet-500/10", border:"border-purple-500/20" },
              ].map(cat => (
          ...
          <div className="mt-8 bg-gradient-to-r from-green-500 to-emerald-600 rounded-3xl p-6 text-white text-center">
            <img src={LOGO_URL} alt="Logo" className="w-12 h-12 rounded-xl mx-auto mb-3 object-cover border-2 border-white/30" />
            ...
            <a href={`https://wa.me/${VENDEUR_WHATSAPP}?text=${encodeURIComponent("Bonjour, j'ai besoin de conseils sur les produits BIEN-ÊTRE YUPI 🌿")}`}
              target="_blank" rel="noreferrer"
              className="inline-flex items-center gap-2 bg-white text-green-600 font-black px-6 py-3 rounded-2xl hover:bg-green-50 shadow">
              <MessageCircle className="w-5 h-5" /> {VENDEUR_DISPLAY}
            </a>
          </div>
        </div>
      )}
